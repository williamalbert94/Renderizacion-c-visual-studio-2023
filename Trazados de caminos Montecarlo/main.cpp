...
#include "opt/esfera/esfera.h"
#include "opt/esfera/moving_sphere.h"
#include "opt/esfera/hitable_list.h"
#include "float.h"
#include "opt/camera/camera.h"
#include "opt/material/material.h"
#include "opt/bvh.h"
#include "opt/box.h"
#include "opt/surface_texture.h"
#include "opt/aarect.h"
#include "opt/material/texture.h"
#include "opt/esfera/pdf.h"
...
int main() {
    ...
    const int ny = 250;
    int ns = 1000;
    float pixels[nx][ny][3];
    string fileName;

    fileName = "test1_1000.ppm";
    ...
    testimage << "P3\n" << nx << " " << ny << endl << nx - 1 << endl;
    for (int j = ny - 1; j >= 0; j--) {
        for (int i = 0; i < nx; i++) {
            for (int k = 0; k < 3; k++)
            {
                testimage << int(pixels[i][j][k] * (nx - 1)) << " ";
            }
            testimage << "\t";
        }
        testimage << "\n";
